...
async function seed() {
    console.log(`≡ƒôí Seeding via API: ${API_URL}`);
    ...
    for (let i = 0; i < randomizedFeed.length; i += 1) {
        ...
        if (!target) {
            console.warn(
                "ΓÜá∩╕Å  Skipping entry: matchId missing or not found:",
                entry.message,
            );
            continue;
        }
        ...
        console.log(`≡ƒôú [Match ${match.id}] ${row.message}`);
        ...
        //     console.log(`≡ƒÅü [Match ${match.id}] Match finished.`);
        //   } else {
